...
func (v *Validator) AddFieldError(key, message string) {
	...
	msgs, exists := v.FieldErrors[key]
	if !exists {
		v.FieldErrors[key] = []string{message}
		return
	}
	v.FieldErrors[key] = append(msgs, message)
}
...
// ExactChars returns true if a value contains at least n characters.
func ExactChars(value string, n int) bool {
	return utf8.RuneCountInString(strings.TrimSpace(value)) == n
}
...
func MinNumber(value string, min int) bool {
	v, err := strconv.Atoi(value)
	return v >= min && err == nil
}

func IsTime(value string, layout string) bool {
